...
### 3. chat_id 자동 학습 (TF-958 hotfix — 사용자 부담 0)

봇은 *어디로 보낼지* 알아야 해요. 갱이가 안내 시점에 봇이 *자동 학습*해서 파일에 박아 둬요. 사용자는 *텔레그램에 메시지 1건만* 보내면 끝.

```
사용자에게 안내:
1. 봇이 켜져 있는지 확인 (./scripts/start.sh 또는 /bot start)
2. 텔레그램에서 봇에게 아무 메시지나 하나 보내 주세요 (예: "안녕")
3. 갱이가 자동 파일 read 로 chat_id 가져옴
```

```bash
# bot.py 가 message 받을 때마다 /tmp/doppel-last-chat-id 에 자동 박음.
# 갱이가 그 파일에서 read.
CHAT_ID=$(cat /tmp/doppel-last-chat-id 2>/dev/null)
test -n "$CHAT_ID" && echo "✅ chat_id 자동 학습됨: $CHAT_ID" || echo "⚠️ 봇에 메시지 1건 보내 주세요"
```

→ 의뢰자가 *수동으로 chat_id 찾을 필요 없음*. /tmp 파일에서 그대로 가져옴.

(fallback — `/tmp/doppel-last-chat-id` 없으면 옛 방식: `grep -E 'incoming chat_id=' /tmp/doppel-bot.log | tail -1 | sed -E 's/.*chat_id=([0-9-]+).*/\1/'`)

### 4. `config/briefing.yml` 의 schedule 섹션 박기

```bash
# chat_id 는 자동 학습 파일에서 read.
CHAT_ID=$(cat /tmp/doppel-last-chat-id 2>/dev/null)
TIME='<사용자_입력_HH:MM>'
