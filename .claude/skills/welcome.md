---
name: welcome
description: 도클갱어 W2 첫 시작 — 갱이가 인사하고 8단계 흐름을 설명하며 환경(Claude Code CLI 로그인·BotFather 안내·Python 버전)을 점검한다. clone 직후 한 번 호출.
---
...
3. **환경 점검 — 다음 3개를 차례로 확인**
   ...
   **3-3. Claude Code CLI 로그인 확인** (TF-938 hotfix — API 키 불필요)
   ```bash
   which claude && claude --version 2>/dev/null | head -1
   ```
   - 경로 + 버전 출력되면 OK. Claude Code 가 *본인 구독 사용량*으로 동작합니다 — 별도 API 키·결제 불필요.
   - 없으면 *멈추고 안내*: "Claude Code 설치·로그인이 먼저예요" (https://docs.claude.com/claude-code/setup).
