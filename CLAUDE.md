...
- **W6**: 본인 도플갱어 전환 준비 + *부가기능* — SNS 자동 게시 ← **TF-1060 누적** (텔레그램 글 → 페르소나 톤 → MBTI SNS 게시)
- **W7~W8**: 본인 도플갱어 + 데모데이
...
⚠️ **TF-1060 W6 부가 (2026-06-25)**: `commands/sns_publish.py` 신설. *기능 1개 추가* — 텔레그램 글 → 페르소나 톤 변환 → **MBTI SNS 자동 게시**. 흐름: `/sns <내용>`(명령 전용 — 자연어 자동 인식 X, 사적 채팅 보호) → 페르소나가 *본인 톤 짧은 SNS 글*로 변환(`generate_sns_post`, 사실 보존) → 텔레그램 **미리보기** + 인라인 [✅ 게시][✖ 취소] → [게시] 눌러야 비로소 **MBTI SNS MCP HTTP** 로 게시(`publish_post` — 3-스텝 핸드셰이크 initialize→initialized→tools/call `mbti_create_post`, SSE 응답 파싱) → post id + `https://mbti.triforge.kr/post/<id>/` 회신. **항상 미리보기 확인 후 게시** (공개·되돌리기 어려운 글이라 확인 게이트 — W4 안전 원칙 정합). 초안은 `context.user_data` 에 토큰 키로 보관(callback_data 64바이트 한도 회피), 게시 시 1회 소비(중복 방지). **새 외부 의존성 0** (requests 재사용), outbound HTTP 1종(MCP)뿐. `MBTI_SNS_API_KEY` 미설정 시 `/sns` 자동 OFF(안내만) — W2~W5 정상 동작. 양식은 `config/sns.yml`(mbti_tag·max_len·style·ack) — 매 호출 reload, 재시작 불필요. ⚠️ **mbti.triforge.kr 는 원탁(외부) 시스템** — 연결만, 그쪽 파일 미변경(격리 원칙).
...
## 3.8 W6 부가 흐름 (TF-1060, 2026-06-25 — SNS 자동 게시)

W6 의 *부가기능*. 본인 도플갱어로 가기 전, 봇이 *글까지* 써 주는 맛보기예요. W2 봇·페르소나는 **그대로** 두고 *기능 1개*만 추가.

0. **셋업** — `/setup-sns` (mbti.triforge.kr 가입 → API 키 발급 → `.env` 의 `MBTI_SNS_API_KEY` + `config/sns.yml` 복사·mbti_tag 설정 → 봇 재시작). 키 = *공개 글 쓰기 권한* 주의.
1. **첫 시연** — `/sns <올리고 싶은 내용>` → 페르소나가 짧은 SNS 글로 변환 → 텔레그램에 *미리보기* + [✅ 게시][✖ 취소].
2. **미리보기 확인** — 페르소나 톤·사실(숫자·이름) 점검 후 [게시] → `post #N` + `https://mbti.triforge.kr/post/N/`. 아쉬우면 [취소] → 폐기.
3. **시연 + 피드 확인** — `/test-sns` ([취소] 도 해보고, mbti.triforge.kr 피드에서 본인 글 확인).

W6 의도: W2 인프라 그대로 + 기능 1개. **항상 미리보기 확인 후 게시**(공개·되돌리기 어려운 글이라 확인 게이트 필수). `/sns` 명령 전용(사적 채팅 보호). 게시(쓰기)는 **MCP HTTP 1종**뿐, 새 외부 의존성 0(requests 재사용), 본인 노트북 100% 완결. mbti.triforge.kr 는 *원탁(외부) 시스템* — 연결만.
...
```
doklgaenger-w2-template/             (W3+W4+W5+W6 누적: TF-958, TF-981, TF-1040, TF-1060 — repo 이름은 유지)
├── README.md
├── CLAUDE.md
├── .env.example                     (W3: ICAL_URL / W4: SLACK_*_TOKEN, TELEGRAM_PUSH_CHAT_ID / W5: 추가 env 0 / W6: MBTI_SNS_API_KEY, MBTI_SNS_MCP_URL)
├── .gitignore                       (W3: config/briefing.yml / W4: config/slack_mention.yml / W5: config/meeting_notes.yml / W6: config/sns.yml)
├── pyproject.toml                   (W3 deps + W4: slack-bolt / W5: 새 dep 0 / W6: 새 dep 0 — requests 재사용)
├── persona/
│   ├── system_prompt.md.example
│   └── pick-guide.md
├── bot.py                           (W3: /today + 자연어 + JobQueue / W4: Socket Mode task / W5: /notes + 회의록 라우팅 + 분할발송 / W6: /sns + 미리보기 + CallbackQueryHandler 게시/취소)
├── commands/                        ← (W3 신규)
│   ├── __init__.py
...
│   └── sns_publish.py               ← (W6 신규 — SNS 글 변환 + MCP 3-스텝 핸드셰이크 게시 + yml reload)
├── config/
...
│   └── sns.yml.example              ← (W6 신규 — enabled·mbti_tag·max_len·style·ack)
├── scripts/
│   ├── start.sh
│   └── stop.sh
├── .claude/
│   ├── settings.json
│   └── skills/                      (W2 8개 + W3 4개 + W4 5개 + W5 5개 + W6 2개 = 24개)
...
│       ├── setup-sns.md                   ← (W6 신규 — SNS 게시 셋업)
│       └── test-sns.md                    ← (W6 신규 — SNS 게시 시연)
└── docs/
    ├── botfather-guide.md
    ├── troubleshoot.md
...
14. **공개 SNS 게시는 항상 미리보기 확인 후** (W6) — `/sns` 는 *미리보기 → [게시] 버튼*을 거쳐야 비로소 게시. 미리보기를 건너뛰고 바로 게시하는 경로를 추가하지 않기. 확인 게이트는 공개·되돌리기 어려운 게시의 안전장치예요(W4 안전 원칙 정합).
15. **사적 채팅 자동 게시 금지** (W6) — `/sns` *명령 전용*. 자연어·일반 메시지를 SNS 게시로 자동 인식하는 트리거를 추가하지 않기. 사적인 채팅이 멋대로 공개되는 사고 방지.
16. **MBTI SNS API 키 공유 금지** (W6) — `MBTI_SNS_API_KEY` 는 *공개 글 쓰기 권한*. `.env`(.gitignore)에만, 공유·캡처·로그 노출 금지. 분실 시 mbti.triforge.kr 에서 재발급.
17. **mbti.triforge.kr 는 원탁(외부) 시스템 — 연결만, 우리 파일 미변경** (W6) — 게시는 그쪽 MCP HTTP 를 *호출*만 해요. 원탁 코드·파일을 건드리거나, 거기에 회사 코드명·제3자 실명·민감정보를 게시하지 않기 (격리 원칙).
18. **`config/sns.yml` 도 git에 올리지 않기** (W6) — 개인 mbti_tag·톤 설정. `.gitignore`에 포함됨.
...
### 8.4 W6 운영 메모 (TF-1060 — SNS 자동 게시)

- **명령 전용 + 확인 게이트**: `/sns <내용>` 만 트리거 (자연어 자동 인식 X — 사적 채팅 보호). 변환 → *미리보기* + 인라인 [✅ 게시][✖ 취소] → [게시] 눌러야 비로소 게시. *항상 확인 후* — 공개·되돌리기 어려운 글이라 (W4 "봇이 멋대로 발송 안 함" 안전 원칙 정합).
- **초안 보관**: 변환 초안은 `context.user_data["sns_drafts"]` 에 12자 토큰 키로 보관, callback_data 엔 `sns:publish:<token>`/`sns:cancel:<token>` 만 (텔레그램 callback_data 64바이트 한도 회피). 게시·취소 시 `pop` 으로 1회 소비 → *중복 게시 방지*. 토큰 20개 초과 시 오래된 것 정리(메모리 cap). 봇 재시작 시 user_data 휘발 → "초안 못 찾음" 안내 후 `/sns` 재시작 유도.
- **사실 보존**: 페르소나는 메모를 짧은 SNS 글로 *톤만* 입히고 숫자·고유명사는 *지어내지 못함* — `build_sns_system_prompt` 의 지침이 명시(W3/W5 정합). 출력은 *게시글 본문만*(메타 코멘트·따옴표·헤딩 금지).
- **MCP 3-스텝 핸드셰이크**(`publish_post`, 동기 → bot 에선 `asyncio.to_thread`): (1) `initialize` → 응답 헤더 `mcp-session-id` 확보, (2) `notifications/initialized`(헤더 `Mcp-Session-Id`) → 202, (3) `tools/call` `mbti_create_post`(헤더 `Mcp-Session-Id`). 공통 헤더 `Authorization: Bearer <key>` + `Accept: application/json, text/event-stream`. 응답은 SSE — `data:` 줄의 JSON 을 모아 `id`/`result` 있는 것 취함. 결과 `result.structuredContent.post = {id, author, author_mbti, content, mbti_tag, ...}`. timeout 30s. 401 = 키 만료 친절 안내.
- **sns.yml reload**: `load_sns_config` 매 `/sns` 마다 호출 — `/setup-sns` 변경 시 *재시작 불필요*, 다음 글부터 즉시 반영. 단 *코드*(`/sns` 등록)는 봇 재시작 1회 필요(setup-sns step 4).
- **미설정 시 자동 OFF**: `MBTI_SNS_API_KEY` 미설정이면 `/sns` 가 `/setup-sns` 안내만(silent skip) — W2~W5 정상 동작. yml `enabled:false` 도 OFF.
- **외부 격리**: mbti.triforge.kr 는 *원탁(외부) 시스템*. 게시는 그쪽 MCP HTTP 호출만 — 우리 파일·원탁 파일 미변경. outbound HTTP 1종(MCP)뿐, 새 외부 의존성 0(requests 재사용).
- **로그**: post_id·token·취소만 찍힘. 게시 본문·API 키는 로그에 안 찍힘.
