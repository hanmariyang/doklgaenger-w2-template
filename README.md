# 도클갱어 W2/W3/W4 — 본인 톤 첫 챗봇 + 캘린더 아침 브리핑 + 슬랙 멘션 답변 어시스턴트

> 안녕하세요, 갱이예요 🐾
> 갱이가 8주 도클갱어 스터디의 **W2(본인 톤 첫 챗봇) + W3(캘린더 아침 브리핑) + W4(슬랙 멘션 답변 어시스턴트)**을 함께 만들어 드릴게요.

W2는 *임의의 영화/캐릭터 페르소나*를 골라서 텔레그램 봇으로 5문답까지 굴려 보는 한 주예요. W3는 그 봇에 *기능 1개*만 더해요 — 매일 아침 페르소나가 오늘 일정을 그려서 보내 주는 캘린더 브리핑. W4는 또 *기능 1개* — 본인이 슬랙에서 멘션받으면 페르소나가 답변 초안을 만들어서 본인 텔레그램으로 흘려 줘요 (슬랙에는 답신 안 함, 본인 손으로). 본인 도플갱어 전환은 W7~W8에 할 거니까, W2~W4는 가볍게 즐기는 구간이에요.
...
## W3/W4 의존성 (clone 직후 1회)

```bash
~/.local/bin/uv sync           # icalendar, python-dateutil, pyyaml, requests, PTB[job-queue], slack-bolt
```

설치 후 Claude Code 안에서 `/welcome-w3` 또는 `/welcome-w4` 호출하면 갱이가 안내해 드려요. W3·W4 Skill 은 모두 `.claude/skills/` 에 박혀 있어요.
