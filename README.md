# 도클갱어 W2/W3/W4/W5 — 본인 톤 첫 챗봇 + 캘린더 아침 브리핑 + 슬랙 멘션 답변 + 회의록 요약 (+W6 부가: SNS 게시)

> 안녕하세요, 갱이예요 🐾
> 갱이가 8주 도클갱어 스터디의 **W2(본인 톤 첫 챗봇) + W3(캘린더 아침 브리핑) + W4(슬랙 멘션 답변 어시스턴트) + W5(회의록 요약 어시스턴트)** + **W6 부가(SNS 자동 게시)**를 함께 만들어 드릴게요.

W2는 *임의의 영화/캐릭터 페르소나*를 골라서 텔레그램 봇으로 5문답까지 굴려 보는 한 주예요. W3는 그 봇에 *기능 1개*만 더해요 — 매일 아침 페르소나가 오늘 일정을 그려서 보내 주는 캘린더 브리핑. W4는 또 *기능 1개* — 본인이 슬랙에서 멘션받으면 페르소나가 답변 초안을 만들어서 본인 텔레그램으로 흘려 줘요 (슬랙에는 답신 안 함, 본인 손으로). W5는 *가장 단순한* 기능 1개 — 회의 메모를 텔레그램에 붙여 넣으면 페르소나가 구조화 회의록(요약·논의·결정·액션·질문)으로 정리해 줘요 (음성·STT 없이 텍스트 한 가지). W6 부가는 *글쓰기* — `/sns 내용` 으로 메모를 던지면 페르소나가 *본인 톤 짧은 SNS 글*로 다듬어 텔레그램에 *미리보기*로 보여주고, [게시] 버튼을 눌러야 비로소 MBTI SNS(mbti.triforge.kr)에 올라가요 (확인 후 게시 — 사적 채팅 자동 게시 금지). 본인 도플갱어 전환은 W7~W8에 할 거니까, W2~W6은 가볍게 즐기는 구간이에요.
...
| ★ | **자유 실습 (선택)** | `/save-notes-db` — 무상태였던 봇에 *회의록 DB 저장*을 직접 붙여 보기. SQLite/Postgres/Notion… 본인 DB로. 결승선 아님, 하고 싶은 분만 |

W5 핵심: *적은 인풋(텍스트 붙여넣기 하나), 높은 아웃풋(구조화 회의록)*. 음성·STT·파일 0, 새 외부 의존성 0, 본인 노트북 100% 완결. **사실 보존** — 페르소나는 톤만 입히고 참석자·결정·숫자·날짜는 그대로.

**자유 실습** (`/save-notes-db`): 지금까지 봇은 *무상태*였어요. 더 해보고 싶으면 생성된 회의록을 본인 DB에 저장하는 *저장 계층*을 직접 구현해 봐요 — 훅 지점(`commands/notes_store.py` + `config` 의 `store.enabled`)은 비워 뒀고, 갱이가 설계 질문·힌트만 거들어요. 정답 코드는 본인 손으로.

- `/notes` 뒤에 메모를 붙이거나, "회의록"·"정리해줘" 같은 자연어, 또는 긴 메모(600자·여러 줄)를 그냥 보내면 자동 인식.
- 긴 회의록은 텔레그램 4096자 한도 때문에 *여러 메시지로 분할 발송* — (1/3),(2/3) 처럼 표시.

## W6 부가 — SNS 자동 게시 (TF-1060, 졸업 부가기능)

W2 인프라(텔레그램 봇 + 페르소나) **그대로** + *기능 1개*. 본인 도플갱어로 가기 전, 봇이 *글까지* 써 주는 맛보기예요.

| # | 단계 | 갱이가 도와드리는 거 |
|---|------|-------------------|
| 0 | 셋업 | `/setup-sns` — mbti.triforge.kr 가입 + API 키 발급 → `.env` + `config/sns.yml` + 봇 재시작 |
| 1 | 첫 시연 | `/sns <내용>` → *미리보기* + [✅ 게시] [✖ 취소] 도착 |
| 2 | 미리보기 확인 | 페르소나 톤·사실 점검 후 [게시] → post 번호 + `https://mbti.triforge.kr/post/N/` |
| 3 | 시연 + 피드 확인 | `/test-sns` — [취소] 도 해보고, 피드(mbti.triforge.kr)에서 본인 글 확인 |

W6 핵심: **항상 미리보기 확인 후 게시** (공개·되돌리기 어려운 글이라 확인 게이트 필수 — W4 의 "봇이 멋대로 공개 발송 안 함" 안전 원칙 정합). `/sns` *명령 전용* (자연어 자동 인식 X — 사적 채팅 보호). 게시(쓰기)는 **MCP HTTP** 1종뿐, 새 외부 의존성 0 (requests 재사용). `MBTI_SNS_API_KEY` 미설정 시 자동 OFF — W2~W5 정상 동작.

- `/sns` 뒤에 올리고 싶은 내용을 적으면 페르소나가 *본인 톤 짧은 글*(기본 280자)로 변환 → 미리보기.
- **mbti.triforge.kr 는 원탁(외부) 시스템** — 연결만, 그쪽 파일 미변경. API 키·민감정보 게시 금지.
...
- **W6 부가**: mbti.triforge.kr 계정 + API 키 1개 (`/setup-sns` 안내). 새 패키지 0 (requests 재사용). 미설정 시 `/sns` 자동 OFF.
